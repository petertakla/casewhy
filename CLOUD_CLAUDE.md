...
> **Last updated by Claude Code: Sep 5, 2026, ~1:10am ET.** If you (cloud session) are reading a copy of this file fetched before that time, re-fetch it now before editing — an earlier stale-copy edit reverted two already-resolved sections back to "unresolved" once already tonight (see the process note under "Right now" below). Tonight's session, in order: shipped the plain-language explanation layer (fixing an AI Gateway 403 along the way), then persistent case tracking + magic-link auth (Neon Auth), then encrypted `tracked_cases.receipt_number` at rest per the standing compliance guardrail, then a file upload UI stub, then fixed `npm run lint` and added it to CI, then built and scheduled the email-notification infrastructure (polling/diff/send logic + a live daily cron-job.org trigger — real sending still needs a Postmark account, see below). Every "Next build priorities" item is now done except that one Postmark signup. All live on `casewhy-app.vercel.app`, CI green (typecheck + lint + build), everything pushed to `nextjs-app`. **Also new tonight: the user wants all CaseWhy email, sent or received, to use `info@casewhy.com` — not their personal address.** Relatedly: Claude Code's Gmail tool access here is tied to the user's personal Gmail, same as the cloud session's own connector — recommended fix is Gmail delegation (add the personal address as a delegate on `info@casewhy.com`, Gmail settings → Accounts) rather than a second connector, since Claude Code doesn't have a "Cowork"-style multi-connector picker. Not yet done as of this writing.
...
- **Email notification infrastructure: built Sep 5, 2026 (past midnight) — code done, cron scheduling done, real sending still needs one external account.** `tracked_cases` extended with encrypted `email` (captured from `session.user.email` at track-time) and `lastStatusText`, plus plain `lastCheckedAt`. New `POST /api/cron/check-status` (`src/app/api/cron/check-status/route.ts`), protected by `Authorization: Bearer <CRON_SECRET>`: for each tracked case, calls the existing `getCaseStatus()`, compares to the last-seen status, and on a change sends via a new Postmark HTTP wrapper (`src/lib/email/postmark.ts`, plain `fetch`, no SDK) — but never on the *first* check after tracking (the user already saw that status when they tracked it; a notification then would be redundant). Switching a tracked case to a different receipt number resets the notification baseline, so a stale status from the old case can't fire a spurious email for the new one.
  ...
  - **Only one thing left to make this actually send real email:** a **Postmark** account (recommended over SendGrid — transactional-only, better deliverability fit for this exact use case), with `casewhy.com` verified as a sending domain (a couple of DNS records; Claude Code can add those directly at Cloudflare once there, they're purely additive) and the Server API Token added as `POSTMARK_API_TOKEN` the same way every other secret got added tonight. Account creation itself is the one step only the user can do.
...
5. ~~**Email notifications** (SendGrid or Postmark) on a detected status change.~~ — **Done Sep 5, 2026, past midnight**, see Product build above. Code, cron scheduling, and test-fire all verified. **START HERE next session:** only remaining step is a Postmark account signup + domain verification (user-only) — everything else is built.
...
17. Create the Postmark account + verify `casewhy.com` as a sending domain — the one remaining step to make notifications actually send real email, user-only (account creation). See Product build above for exact detail.
18. Optional, not blocking: set up Gmail delegation (`ptakla@gmail.com` as a delegate on `info@casewhy.com`) so Claude Code's existing Gmail tool access can also see/send as the business address when needed for testing.
