...
- **Email notification infrastructure: built Sep 5, 2026 (past midnight) — code is done and verified, real sending is not live yet, needs two external accounts.** `tracked_cases` extended with encrypted `email` (captured from `session.user.email` at track-time) and `lastStatusText`, plus plain `lastCheckedAt`. New `POST /api/cron/check-status` (`src/app/api/cron/check-status/route.ts`), protected by `Authorization: Bearer <CRON_SECRET>`: for each tracked case, calls the existing `getCaseStatus()`, compares to the last-seen status, and on a change sends via a new Postmark HTTP wrapper (`src/lib/email/postmark.ts`, plain `fetch`, no SDK) — but never on the *first* check after tracking (the user already saw that status when they tracked it; a notification then would be redundant). Switching a tracked case to a different receipt number resets the notification baseline, so a stale status from the old case can't fire a spurious email for the new one.
  ...
  - **Two things only the user can do to make this actually send real email:** (1) create a **Postmark** account (recommended over SendGrid — transactional-only, better deliverability fit for this exact use case), verify `casewhy.com` as a sending domain (a couple of DNS records; Claude Code can add those directly at Cloudflare once there, they're purely additive), hand over the Server API Token as `POSTMARK_API_TOKEN` the same way every other secret got added tonight. (2) create a **dedicated cron-job.org account under `info@casewhy.com`** (not the NVDA trading project's account — that one's already at its 7-job free-tier cap, and this keeps CaseWhy's infra cleanly separate) and add one daily job hitting `/api/cron/check-status` with `Authorization: Bearer <CRON_SECRET>` — matching the NVDA project's own hard-won lesson, the literal word `Bearer ` (with trailing space) must precede the token or the header arrives empty.
...
5. ~~**Email notifications** (SendGrid or Postmark) on a detected status change.~~ — **Infrastructure done Sep 5, 2026, past midnight**, see Product build above. **START HERE next session:** the only remaining work is two external account setups only the user can do (Postmark + a dedicated cron-job.org account) — everything else is built and verified. Not "code work" in the usual sense; more a checklist to walk through together.
...
16. Create the Postmark account + verify `casewhy.com` as a sending domain, and a dedicated cron-job.org account under `info@casewhy.com` — the two remaining steps to make notifications actually send/fire, both user-only (account creation). See Product build above for exact detail.
17. Optional, not blocking: set up Gmail delegation (`ptakla@gmail.com` as a delegate on `info@casewhy.com`) so Claude Code's existing Gmail tool access can also see/send as the business address when needed for testing.
