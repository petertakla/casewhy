...
**Round 130** — the actual USCIS API Demo Checklist, sent with the scheduling link, is broader than those five numbered criteria and adds real, separate asks this doc now also answers: the browser's own DevTools Network tab specifically, CaseWhy's own frontend/backend authentication as its own point (distinct from USCIS OAuth), and manually running the scheduled batch job live. See "App authentication," the Network-tab question under "JSON payload handling," and "Batch/scheduled updates," below.
...
Neon Auth, supporting password and magic-link sign-in — a real session between the browser and our server, checked on every request that touches account or case data (tracking a case, checking status on demand, account settings). This is a completely separate relationship from the one below: it's the browser talking to *us*. The OAuth 2.0 flow in the next section is *us* talking to *USCIS* — two different authentications, not one. *Shown in demo: step 1.*

**Why does that distinction matter for this demo?**
Because the checklist asks for both, as separate lines — "authentication between the frontend and backend of your application" (this) and "OAuth 2.0" (next section) are different requirements, not two ways of describing the same thing.
...
Yes — the on-demand check button has a "Show technical details" toggle that renders the exact outbound request (method, URL, headers, with the token redacted) our code sends for that click. It's generated by the same function `getCaseStatus()` itself uses, not a second hand-written description that could drift out of sync. *Shown in demo: step 2.*

**Can you show this in the browser's own DevTools Network tab, since that's specifically requested?** Round 130.
Honestly: the request to USCIS itself, no — our OAuth credentials and the USCIS API call run entirely server-side, by design, specifically so a client ID, secret, or access token can never reach the browser. That means the raw USCIS request genuinely never crosses the browser's own network layer, so DevTools can't show it no matter how the app is built. What DevTools *does* show, and what I'll point to live: the request from the browser to our own server that triggers that server-side call — the browser-to-us leg is real and visible; the us-to-USCIS leg is shown instead via the "Show technical details" panel above, built from the same code that makes the real call. *Shown in demo: step 2.*
...
Sign in, enter a receipt number, done. Free accounts get 3 receipt numbers for the life of the account — tracking one or just looking up its status both count against that same cap, and untracking a case doesn't free a slot back up. Paid accounts: more, in tiers, with a hard ceiling above which we talk to the user directly rather than self-serve. *Shown in demo: step 2 (adding a new case live).*

**How do you detect a status change, and how does the user find out?**
Our scheduled job compares the freshly fetched status text against the last one we saw for that case. On a genuine change (never on the very first check), we send an email via Postmark and, if the user's enabled it, a push notification. *Shown in demo: step 5, and run manually live in step 6.*
...
Yes — once a day, on a fixed schedule, every day. It re-checks every actively tracked case against USCIS and sends the notification described above on a genuine change.

**Can you run it live, right now, not just describe it?**
Yes — it's a real endpoint (`/api/cron/check-status`), normally called by an external scheduler on a bearer-token header, the same one an admin diagnostic tool uses to trigger and verify real deployed routes. I can call it by hand with the identical header, live, right now. *Shown in demo: step 6.*

**How does it handle one case failing without breaking the whole run?**
Each tracked case is checked independently, inside its own try/catch — a failure on one case is recorded and the loop continues to the next, not aborted. The response reports exactly how many succeeded, how many failed and why, and how many are left if the run's own time budget ran out before finishing the full list. *Shown in demo: step 6, with a deliberately-unrecognized receipt number tracked specifically to produce a real error in the same run as real successes.*

**Is that deliberately-broken test case going to look bad, like something's wrong with the app?**
No — it's tracked on purpose, precisely because it reliably reproduces a real USCIS 404 on demand, so the error-handling path can be shown live rather than described. It's the same code path a genuinely unrecognized case would hit in production, working exactly as intended: reported cleanly, isolated from the cases that succeed.
