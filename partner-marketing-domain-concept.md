...
**Status, updated Sep 8 (checked against `CLOUD_CLAUDE.md`'s own build log, latest pass): Attorneys (round 27/28), Accredited Representatives (round 29, reworked/seeded in round 32, nav-corrected in round 33), and the Get Help hub + full nav surfacing (round 31) are all built, seeded, and live — all three gaps found on the previous check (empty directory, missing "free" messaging, nav placement) are now resolved, confirmed against rounds 32/33's own done-notes rather than assumed. Legal aid / nonprofit organizations (entity type 3) is now authorized as round 34, built+seeded in one pass this time.** Entity types 4-6 remain unauthorized — need an explicit go-ahead each. Two decisions Peter made explicitly (Sep 8), superseding the first draft of this doc: **(1) all of Track A, B, and C become real, in-app, user-facing entities inside CaseWhy itself** — not just an outbound marketing funnel that recruits partners who then sit off to the side; **(2) each entity type gets its own fully separate system** (own table, own public page, own application/intake flow) rather than one shared directory filtered by type. This is a bigger build than the original single-directory version, so it's broken into a sequenced backlog rather than one round.
...
- **Every entry gets its own permalink** — `/<type>/<slug-or-id>` — not just a card on the `/<type>` index page. Decided Sep 8: applies to all six entity types uniformly, no exceptions. Confirmed live for both entity types built so far (`/attorneys/[id]`, `/accredited-representatives/[slug]`, added in round 32).
...
2. **BIA-accredited representatives** — non-lawyer, DOJ-authorized representatives, typically at nonprofits. Same shape as attorneys (name, organization, credential info in place of a bar number, practice focus, contact) but its own separate table/page/flow per Peter's call, not a field on the attorney form. **Live at `/accredited-representatives` (round 29, reworked/seeded round 32, nav-corrected round 33) — 98 real DOJ-sourced records, Florida-first.**
3. **Legal aid / nonprofit immigration organizations** — reach users who can't afford a private attorney; genuinely press-worthy alongside the founder story. Fields: org name, org type, contact person, population/regions served, services offered. **Round 34, greenlit Sep 8, in progress — see `round34-legal-aid-orgs-build-and-seed-task.md`.**
...
**Recommended build order after round 34:** university DSOs (4) and community orgs (5) next, in either order, with employers (6) last since it depends on a product decision (team accounts) that doesn't exist yet.
...
Attorneys (round 27/28) and accredited representatives (round 29/32/33) are fully built, seeded, and live — no known gaps as of this check. Legal aid orgs (entity type 3) is authorized and in progress as round 34 — see `round34-legal-aid-orgs-build-and-seed-task.md`. Entity types 4-6 (DSOs, community orgs, employers) remain unauthorized, each needs its own explicit go-ahead, and none has a workable baseline data source identified yet.
