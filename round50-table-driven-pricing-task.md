...
**Round 118 update, Sep 19/20, 2026: the original 3-month tier was retired in favor of a 6-month tier, and the annual price was bumped. The figures below are updated to match the live pricing; the pre-round-118 numbers are preserved in git history if ever needed.**
...
3. **No refund/proration policy of any kind.** A subscriber who cancels a 6-month or annual plan mid-period gets no partial refund — same "reverts to free tier at the end of the current paid period, not instantly" behavior round 13 already built for monthly, just confirmed to apply identically to the new 6-month/annual plans. Nothing new to build here, but see the disclosure-copy note below — this needs to be stated plainly to the buyer before purchase, not just true in the backend. (Round 118: plan switching now prorates automatically — see that round's own notes.)
4. Final prices (round 118): **monthly $9.99** (unchanged), **6-month $39.99** (≈ $6.67/mo, ~33% off the monthly-equivalent rate), **annual (12-month) $69.99** (≈ $5.83/mo, ~42% off the monthly-equivalent rate).
...
## Part 1 — base-price table + 6-month/annual plans

### Schema
- `plan_prices` table: `plan_id` (`plus_monthly` | `plus_6month` | `plus_annual`), `base_price_cents`, `billing_interval` (`month` | `year`), `interval_count` (`1` for monthly/annual, `6` for the 6-month plan — Stripe's recurring interval only supports day/week/month/year with a count, so "6 months" is `interval: month, interval_count: 6`). Seed with the three plans above.
...
- `startCheckout` (`src/app/plus/actions.ts`) takes a `plan` parameter (`plus_monthly` | `plus_6month` | `plus_annual`) instead of assuming monthly.
...
### `/plus` page changes
- Add a plan selector: monthly $9.99 / 6-month $39.99 (~$6.67/mo) / annual $69.99 (~$5.83/mo) — show both the period price and the per-month equivalent for each, so the discount is legible at a glance.
...
- Confirm in Stripe's dashboard (test mode) that Coupons and Promotion Codes can actually be created and redeemed end-to-end against the 6-month/annual plans too, not just the existing monthly one.
