...
export async function generateMetadata({
  ...
  return es
    ? pageMetadata("/news?lang=es", {
        title:
          "Noticias de Inmigración — Actualizaciones de USCIS y Políticas | CaseWhy",
        description:
...
function formatDate(date: Date | null, es: boolean): string {
  if (!date) return "";
  return date.toLocaleDateString(es ? "es" : "en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
...
export default async function NewsPage({
  ...
  const enabledSourceIds = await getEnabledNewsSourceIds(
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-10">
      ...
      <h1 className="text-2xl font-bold tracking-tight">
        {es ? "Noticias de inmigración" : "Immigration news"}
      </h1>
          ...
          <>
            ...
            {es ? (
              <>
                ...
                <a
                  href={es ? "/settings?lang=es" : "/settings"}
      ...
      <div className="mb-8">
        <ShareButton
          ...
          text={
            es
              ? "Noticias curadas de USCIS y ley de inmigración, gratis, con CaseWhy."
              : "Curated USCIS and immigration-law news, for free, with CaseWhy."
          }
          es={es}
        />
      </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border-strong p-6 text-center text-sm text-muted">
          {es
            ? "Ninguna noticia disponible en este momento — vuelve a revisar pronto."
        ...
        <>
          <PageFilter
            basePath="/news/"
            placeholder={
              es ? "Filtrar estas noticias…" : "Filter these stories…"
            }
            noMatchText={
              es
                ? "Ninguna noticia en esta página coincide."
                : "No stories on this page match."
            }
            isSpanish={es}
          />
              ...
              >
                ...
                >
                  <p className="text-xs font-semibold uppercase tracking-widest text-muted">
                    {item.sourceName}
                    {item.publishedAt && (
                      <span> · {formatDate(item.publishedAt, es)}</span>
                    )}
                  </p>
