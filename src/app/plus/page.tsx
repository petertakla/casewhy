...
const PLUS_FEATURES: PlusFeature[] = [
  ...
  },
  {
    ...
    plus: "Unlimited",
    explanation:
      "A free account gets 3 receipt numbers for the life of the account — tracking one or just looking up its status both count, and untracking a case doesn't free up a new slot. Fine for a single, simple case, but bigger or more complex households need more: a spouse's I-485 alongside your own, kids' derivative cases, or parents you're helping through an N-400 you understand better than they do. Plus is unlimited case tracking for your family — no fixed cap, no lifetime limit — so the whole household lives under a single login instead of everyone creating separate accounts (and separate $9.99/mo subscriptions) just to see their own status. Each case gets its own full CaseWhy experience — its own status history, its own AI explanations, its own document vault — switched between with a single click, not re-entered from scratch every time. Tracking more than 10 cases at once is uncommon enough that we ask for a quick, one-time check before it kicks in — most households never see this step, and it's a review, not a denial.",
  },
  {
    id: "on-demand-checks",
...
export default async function PlusPage({
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-10">
      <LanguageSwitcher es={false} href="/es/plus" />
