...
export async function generateMetadata({
  ...
  return es
    ? pageMetadata("/visa-bulletin?lang=es", {
...
function MovementBadge({
  ...
  movement: BulletinMovement | null;
  es: boolean;
}) {
  ...
  if (movement === "forward") {
    return (
      <span
        ...
        title={
          es ? "Avanzó desde el mes pasado" : "Moved forward since last month"
        }
      >
        ▲
      </span>
  ...
  if (movement === "retrogressed") {
    return (
      <span
        ...
        title={
          es
            ? "Retrocedió desde el mes pasado"
            : "Retrogressed since last month"
        }
      >
        ▼
      </span>
  ...
  return (
    <span
      ...
      title={
        es ? "Sin cambio desde el mes pasado" : "No change since last month"
      }
    >
      —
    </span>
...
function BulletinTable({
  ...
  rows: BulletinRow[];
  previousRows?: BulletinRow[];
  es: boolean;
}) {
  const previousFor = (category: string) =>
    previousRows?.find((r) => r.category === category);
      ...
      <table className="w-full text-left text-sm">
        <thead>
          <tr className="border-b border-border bg-surface-2 text-xs uppercase tracking-widest text-muted">
            ...
            <th className="px-4 py-3 font-semibold">
              {es ? "Todos los demás países" : "All other countries"}
            </th>
            <th className="px-4 py-3 font-semibold">
              {es ? "China" : "China"}
            </th>
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3 font-mono">
                  ...
                  <MovementBadge
                    movement={computeMovement(row.allOther, prev?.allOther)}
                    es={es}
                  />
                </td>
                <td className="px-4 py-3 font-mono">
                  ...
                  <MovementBadge
                    movement={computeMovement(row.china, prev?.china)}
                    es={es}
                  />
                </td>
                <td className="px-4 py-3 font-mono">
                  ...
                  <MovementBadge
                    movement={computeMovement(row.india, prev?.india)}
                    es={es}
                  />
                </td>
                <td className="px-4 py-3 font-mono">
                  ...
                  <MovementBadge
                    movement={computeMovement(row.mexico, prev?.mexico)}
                    es={es}
                  />
                </td>
                <td className="px-4 py-3 font-mono">
                  {row.philippines
                    ? bulletinDateLabel(row.philippines, es)
                    : "—"}
                  <MovementBadge
...
function summarizeMovement(
  ...
  for (const row of rows) {
    ...
    if (!prev) continue;
    (["allOther", "china", "india", "mexico", "philippines"] as const).forEach(
      (col) => {
  ...
  if (forward === 0 && retrogressed === 0)
    return "No categories moved since last month's bulletin.";
  const parts: string[] = [];
...
function summarizeMovementEs(
  ...
  if (forward === 0 && retrogressed === 0)
    return "Ninguna categoría avanzó desde el boletín del mes pasado.";
  const parts: string[] = [];
...
export default async function VisaBulletinPage({
  ...
  const movementSummary = VISA_BULLETIN_PREVIOUS_MONTH
    ? summarizeMovement(
        ...
        [
          ...(VISA_BULLETIN_PREVIOUS_MONTH.family ?? []),
          ...(VISA_BULLETIN_PREVIOUS_MONTH.employment ?? []),
        ],
      )
    : "";
  const movementSummaryEs = VISA_BULLETIN_PREVIOUS_MONTH
    ? summarizeMovementEs(
        [...FAMILY_FINAL_ACTION, ...EMPLOYMENT_FINAL_ACTION],
        [
          ...(VISA_BULLETIN_PREVIOUS_MONTH.family ?? []),
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: es
          ? "¿Qué significa 'Vigente' en el boletín de visas?"
          : "What does 'Current' mean on the visa bulletin?",
        acceptedAnswer: {
          "@type": "Answer",
          text: es ? CURRENT_MEANS_ANSWER_ES : CURRENT_MEANS_ANSWER,
        },
      },
    ],
  };
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-10">
      ...
      <LanguageSwitcher
        es={es}
        href={localeToggleHref("/visa-bulletin", {}, es)}
      />
      <h1 className="text-2xl font-bold tracking-tight">
        {es ? "Boletín de visas" : "Visa bulletin"}
      </h1>
      <p className="mb-2 mt-2 text-muted">
        {es ? (
          <>
            Fechas de Acción Final — la tabla que determina cuándo una tarjeta
            verde basada en familia o empleo puede realmente emitirse, o un
            ajuste de estatus aprobarse, una vez que una petición ha sido
            aprobada y una fecha de prioridad está esperando a que una visa esté
            disponible.
          </>
        ) : (
          <>
            Final Action Dates — the chart that determines when a family- or
            employment-based green card can actually be issued or adjustment of
            status approved, once a petition is approved and a priority date is
      ...
      <p className="mb-2 rounded-lg border border-border-strong bg-surface-2 p-3 text-sm text-foreground/90">
        {es ? CURRENT_MEANS_ANSWER_ES : CURRENT_MEANS_ANSWER}{" "}
        {es ? movementSummaryEs : movementSummary}
      </p>
      <p className="mb-8 text-xs text-muted">
        ...
        >
          {es
            ? "ver el boletín oficial del Departamento de Estado"
      ...
      <div className="mb-8">
        <ShareButton
          ...
          text={
            es
              ? "Rastrea el movimiento del boletín de visas gratis con CaseWhy."
              : "Track visa bulletin movement for free with CaseWhy."
          }
          es={es}
        />
      </div>
      ...
      <PageFilter
        ...
        noMatchText={
          es
            ? "Ninguna categoría en esta página coincide."
            : "No categories on this page match."
        }
        isSpanish={es}
      />
      ...
      <BulletinTable
        rows={FAMILY_FINAL_ACTION}
        previousRows={VISA_BULLETIN_PREVIOUS_MONTH?.family}
        es={es}
      />
      ...
      <BulletinTable
        rows={EMPLOYMENT_FINAL_ACTION}
        previousRows={VISA_BULLETIN_PREVIOUS_MONTH?.employment}
        es={es}
      />

      <Link
        href={es ? "/processing-times?lang=es" : "/processing-times"}
      ...
      <p className="mt-8 text-xs text-muted">
        {es ? (
          <>
            &quot;Vigente&quot; significa que las visas están disponibles para
            todos los solicitantes calificados en esa categoría sin importar la
            fecha de prioridad. Una fecha indicada significa que solo los
            solicitantes con una fecha de prioridad anterior a esa fecha tienen
            actualmente una visa disponible. Esta tabla se actualiza
            mensualmente — siempre confirma con el boletín oficial de arriba
            antes de basarte en ella para una decisión de presentación.
          </>
        ) : (
          <>
            &quot;Current&quot; means visas are available to all qualified
            applicants in that category regardless of priority date. A listed
            date means only applicants with a priority date earlier than that
            date currently have a visa available. This table is refreshed
            monthly — always confirm against the official bulletin above before
            relying on it for a filing decision.
          </>
        )}
      </p>
    </main>
