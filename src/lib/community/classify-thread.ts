...
export type ClassifyOutcome =
  | { kind: "relevant"; resourceType: "processing_times" | "policy" | "get_help"; reason: string }
...
const SYSTEM_INSTRUCTIONS = `You're screening one public forum thread for CaseWhy, a USCIS case-status tracking app (never a law firm, never affiliated with USCIS/DHS). CaseWhy has three kinds of real, factual data it can ground a reply in:
...
A thread is only "relevant" if it's a genuine question one of those three could actually help answer well — a processing-time question, "what does this status/policy mean," or "where do I find [legal help / an attorney / a DSO]." A thread that merely mentions USCIS without asking something CaseWhy's real data can answer is NOT relevant.
...
RELEVANT: yes or no
RESOURCE: processing_times, policy, get_help, or none
HOSTILE: yes or no
...
export async function classifyThread(params: {
  ...
  if (!relevant || (resource !== "processing_times" && resource !== "policy" && resource !== "get_help")) {
