...
export interface CaseTypeOption {
  id: string;
  label: string;
  /** Round 80 — dropdown label only, per the task doc's explicit scope
   * ("the dropdown and its category labels"). timelineBlurb stays English
   * regardless of locale — it's substantive, legal-adjacent guidance
   * content (like the KB policy memos), not simple UI chrome, and wasn't
   * listed in scope. */
  labelEs?: string;
...
// Round 106
const i131AdvanceParole = findProcessingTime("I-131").find((e) => e.categoryLabel.startsWith("Advance Parole"));
const i131ReentryRefugee = findProcessingTime("I-131").find((e) => e.categoryLabel.startsWith("Re-entry Permit"));
const i129H1b = findProcessingTime("I-129")[0];
const i821dRenewal = findProcessingTime("I-821D")[0];

export const CASE_TYPES: CaseTypeOption[] = [
  {
    id: "n400",
    label: "Naturalization (N-400)",
    labelEs: "Naturalización (N-400)",
    timelineBlurb: n400Note,
    group: "Citizenship & status",
    groupEs: "Ciudadanía y estatus",
  },
  {
    id: "n600",
    label: "Certificate of Citizenship (N-600)",
    labelEs: "Certificado de Ciudadanía (N-600)",
    timelineBlurb:
      "N-600 is adjudicated by the applicant's local field office, same as N-400 — USCIS's tool has no national figure for it, only individual field offices to pick from; see the Processing Times page for the office locator. N-600 doesn't grant citizenship — it requests proof for someone who is already a U.S. citizen by operation of law (through a citizen parent, either automatically at birth abroad or as a minor under the Child Citizenship Act of 2000).",
    group: "Citizenship & status",
    groupEs: "Ciudadanía y estatus",
  },
  {
  ...
  },
  {
    ...
    groupEs: "Basado en empleo",
  },
  {
    id: "i129",
    label: "Nonimmigrant worker petition (I-129 — H-1B, L-1, O-1, TN, etc.)",
    labelEs: "Petición de trabajador no inmigrante (I-129 — H-1B, L-1, O-1, TN, etc.)",
    timelineBlurb: i129H1b
      ? `Timelines vary a lot by visa classification (H-1B, L-1, O-1, TN, and others). One data point CaseWhy has: an H-1B extension of stay filed in the U.S. (regular processing) is currently taking around ${i129H1b.percentile80Months} months (${i129H1b.office}, as of ${i129H1b.asOf}). Premium processing (an extra fee) guarantees action within 15 business days for most classifications. One important fact regardless of timeline, specific to H-1B: once a new or transfer I-129 petition is properly filed and receipted, you can generally start working for the new employer right away — you don't need to wait for approval — provided you're maintaining valid status (or within the 60-day grace period after your last job ended), the new employer's Labor Condition Application is certified, and you have no history of unauthorized work. If that petition is later denied, work authorization under it ends immediately.`
      : "No fixed national timeline figure sourced yet for I-129 — see the Processing Times page for what CaseWhy has on file.",
    group: "Employment-based",
    groupEs: "Basado en empleo",
  },
  {
    ...
    labelEs: "Documento de Autorización de Empleo (I-765)",
    timelineBlurb: i765PendingI485
      ? `Timelines vary a lot by eligibility category. One data point CaseWhy has: an I-765 based on a pending I-485 adjustment, category (c)(9), is currently taking around ${i765PendingI485.percentile80Months} months (${i765PendingI485.office}, as of ${i765PendingI485.asOf}). Other categories (asylum-based, DACA, OPT/STEM, etc.) can differ significantly — see the Processing Times page for what else CaseWhy has on file.`
  ...
  },
  {
    ...
    labelEs: "Documento de viaje — Permiso Anticipado / Permiso de Reingreso (I-131)",
    timelineBlurb:
      i131AdvanceParole && i131ReentryRefugee
        ? `Advance Parole for a pending I-485 applicant is currently taking around ${i131AdvanceParole.percentile80Months} months (${i131AdvanceParole.office}, as of ${i131AdvanceParole.asOf}). A Re-entry Permit or Refugee Travel Document — USCIS reports these together — is currently taking around ${i131ReentryRefugee.percentile80Months} months. One critical thing to know regardless of timeline: if you're requesting Advance Parole and you leave the U.S. before it's approved and in hand, USCIS considers the I-131 abandoned — and for pending I-485 applicants, an unauthorized departure can jeopardize the adjustment application too, not just the travel document.`
        : "No fixed national timeline figure sourced yet for I-131 — see the Processing Times page for what CaseWhy has on file.",
    group: "Travel & green card",
    groupEs: "Viaje y tarjeta verde",
  },
  {
    id: "i90",
    label: "Green Card renewal/replacement (I-90)",
    labelEs: "Renovación/reemplazo de tarjeta verde (I-90)",
    timelineBlurb: i90
      ? `I-90s are currently taking around ${i90.percentile80Months} months for 80% of cases (${i90.office}, as of ${i90.asOf}). USCIS recommends filing up to 6 months before your card expires. Your filing receipt notice extends your expired card's validity for employment/I-9 purposes — this doesn't change your actual permanent-resident status, which doesn't change while an I-90 is pending.`
